...
l4_pattern = re.compile(r'(.+)[UN](.+)')  # 匹配加交并算式
#l4_pattern = re.compile(r'((?!\(|U|N)+)[U|N](.+)?')  # 匹配加交并算式

mul_sub_pattern = re.compile(r'(-?\d+)(\.\d+)?\*-(-?\d+)(\.\d+)?')  # 匹配乘负数算式
div_sub_pattern = re.compile(r'(-?\d+)(\.\d+)?/-(-?\d+)(\.\d+)?')  # 匹配除负数算式

def min_cal_new(string):
    #print string
    if string.count('U') == 1:
        #print string[:string.find('U')]
        #print string[string.find('U') + 1:]
        # [12,3,5] [4] 是字符串，首先转成list
        lst = bt.union(string[:string.find('U')].split(',') , string[string.find('U') + 1:].split(','))
        return ",".join(lst)
    if string.count('N') == 1:
        #print string[:string.find('N')]
        #print string[string.find('N') + 1:]
        lst = bt.intersection(string[:string.find('N')].split(',') , string[string.find('N') + 1:].split(','))
        return ",".join(lst)


def nomal_numerator_new(string):
...
def nomal_numerator(string):  # 定义计算不带括号的算式计算函数
    if string.count('+') + string.count('*') + string.count('/') == 0 and string[1:].find('-') < 0:
        ...
        string = string.replace('--', '+')
        if string.count('*-') != 0:
            string = string.replace(mul_sub_pattern.search(string).group(),
                                    '-' + mul_sub_pattern.search(string).group().replace('*-', '*'))
        if string.count('/-') != 0:
            string = string.replace(div_sub_pattern.search(string).group(),
                                    '-' + div_sub_pattern.search(string).group().replace('/-', '/'))
...
'''
通过交集、并集组成的集合字符串生成list集合

'''
def getstocklistByFormula(str):
    return l1_analysis(str)
