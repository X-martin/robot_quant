...
import filter_method_test as fimt
import pandas as pd
...
class FactorT(object):
    ...
    def get_val(self, stock_list, date):
        ...
        df = fmt.apply_factor_method(self.method[0], self.bfname, stock_list, date, self.method[1])
...
class FilterT(object):
    ...
    def filter(self, stock_list, date):
        df = fimt.apply_filter_method(self.method[0], self.factor_list, stock_list, date, self.method[1])
